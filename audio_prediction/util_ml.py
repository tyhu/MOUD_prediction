...
def batch_norm(X,batch):
    batch_set = list(set(batch.tolist()))
    Xnorm = np.zeros_like(X)
    for b in batch_set:
        Xnorm[np.where(batch==b)] = stats.zscore(X[np.where(batch==b)],axis=0)
        #Xnorm[np.where(batch==b)] = X[np.where(batch==b)]
